...
export class AuthService {
  ...
  public changeEmail(newEmail: string) {
    ...
    return updateEmail(currentUser, newEmail).then(() =>
      this.userService.setUidRecord(newEmail, currentUser.uid)
    );
  }

  public updateAccount(
    accountData: Parameters<typeof updateProfile>[1],
    photo?: File
  ) {
    const { currentUser } = this.firebaseService.auth;
    if (!currentUser) throw Error('Cannot change account if not logged in.');
    return Promise.resolve(
      photo ? this.uploadAvatar(currentUser.uid, photo) : undefined
    ).then((photoURL) => {
      const data = photoURL
        ? Object.assign(accountData, { photoURL })
        : accountData;
      return Promise.all([
        updateProfile(currentUser, data),
        this.userService.updateUserRecord(currentUser.uid, data),
      ]);
    });
  }
  ...
  public signOutUser() {
    ...
    return this.userService
      ...
      .then(() => signOut(this.firebaseService.auth));
  }
  ...
  public waitForUser() {
    return this.getAuthState().pipe(
      filter((user) => Boolean(user)) as FilteredAuthState
    );
  }
  ...
  public checkIfUserExists(email: string) {
    return fetchSignInMethodsForEmail(this.firebaseService.auth, email).then(
      (result) => {
